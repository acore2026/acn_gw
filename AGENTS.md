...
make run-arf            # ARF only (port 9001)
make run-acf            # ACF only (port 9002)
make run-moqt           # MOQT Relay only (port 9003)
```

### Linting
```bash
make lint               # Runs flake8 (if installed)
...
├── main.py                # Entry point (runs all services)

...
class MOQSubscriber:
    ...
    async def _handle_stream_data(self, protocol, data: StreamData):
        ...
        if data.stream_id == 0:
            ...
            # Data stream - accumulate data until end_stream
            ...
            if data.end_stream:
                ...
                logger.info(
                    f"[STREAM] Processing complete stream: stream_id={data.stream_id}, total_size={len(complete_data)}, hex_preview={complete_data.hex()[:60]}"
                )
    ...
    async def _handle_data_stream(self, stream_id: int, data: bytes):
        ...
        try:
            ...
            logger.info(
                f"[STREAM] Decoded stream_type={stream_type}, expected SUBGROUP_HEADER={StreamType.SUBGROUP_HEADER}, data_preview={data[:20].hex()}"
            )
        ...
        try:
            ...
            while offset < len(data):
                try:
                    ...
                    await self._object_queue.put(received_obj)
                    logger.debug(
                        f"Fetch object received: group={obj_header.group_id}, object={obj_header.object_id}"
                    )

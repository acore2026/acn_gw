...
class MOQRelay:
    ...
    async def _on_quic_stream_data(self, protocol, stream_data: StreamData):
        ...
        if is_control_stream:
            ...
            logger.debug(
                f"Received stream data: client={client.session_id}, stream_id={stream_id}, "
    ...
    async def _handle_data_stream(self, client: ClientSession, stream_data: StreamData):
        ...
        logger.info(
            f"_handle_data_stream: client={client.session_id}, stream_id={stream_id}, "
            f"data_size={len(data)}, publications={client.publications}"
        )
    ...
    async def _handle_publish(self, client: ClientSession, msg: PublishMessage):
        ...
        # Use track name directly (normalize removed for compatibility)
